  ...
  return (
    <div className="max-w-xl mx-auto mt-4">
      <div
        ...
        title="Click o Space para voltear"
      >
        ...
        <div className="mt-4 text-xs text-gray-500 text-text-muted">Pulsa <kbd className="px-1 border rounded bg-card-background border-border">Space</kbd> para voltear · ←/→ para navegar</div>
      </div>
      ...
      {showEN && ( // Show feedback buttons only when English is shown
        <div className="mt-3 flex items-center justify-center gap-2">
          <button onClick={() => handleFeedback('correct')} className="px-3 py-1.5 rounded-xl bg-green-500 text-white">Correcto</button>
          <button onClick={() => handleFeedback('incorrect')} className="px-3 py-1.5 rounded-xl bg-red-500 text-white">Incorrecto</button>
        </div>
      )}
      <div className="mt-3 flex items-center justify-center gap-2">
        <button onClick={() => onSpeakES(current.es)} className="text-xs px-2 py-1 rounded-md border border-gray-300 hover:bg-gray-50 border-border text-text-muted hover:bg-card-background">🔊 ES</button>
        <button onClick={() => onSpeakEN(current.en)} className="text-xs px-2 py-1 rounded-md border border-gray-300 hover:bg-gray-50 border-border text-text-muted hover:bg-card-background">🔊 EN</button>
        <button onClick={() => onFav(current.id)} className={classNames("text-xs px-2 py-1 rounded-md border", favs.has(current.id) ? "bg-yellow-100 border-yellow-300" : "hover:bg-gray-50 border-gray-300 border-border text-text-muted hover:bg-card-background")}>{favs.has(current.id) ? "★" : "☆"}</button>
      </div>
    </div>
  );
}